using System;

namespace Problem02.NumbersNotDivisible
{
    class NumbersNotDivisible
    {
        static void Main()
        {
            //Write a program that enters from the console a positive integer n and prints all the numbers 
            //from 1 to n not divisible by 3 and 7, on a single line, separated by a space.

            Console.Write("Enter a number: ");//prompts the user to enter a number
            int n = int.Parse(Console.ReadLine());// convert to integer

            for (int i = 1; i <=n; i++)//for loop to pass through all the numbers from 1 to n
            {
                if (i % 3 == 0 && i % 7 == 0)// check whether the number is divisible by both 3 and 7
                {
                    Console.Write("{0} ", i);// print this if true
                }
                else
                {
                    Console.WriteLine("There are no number divisible by both 3 and 7 up to number {0}", n);
                    break;
                }
            }
            Console.WriteLine();
        }
    }
}
